...
class RegisterResolver {
  ...
  async registerUserWithGoogle(
    ...
    try {
      ...
      let user =
        emailType === "teacher"
          ? await prisma.teacher.findUnique(userQueryParams)
          : await prisma.student.findUnique(userQueryParams);

      let userType: BaseRegistrationResponse["userType"] =
        emailType === "teacher"
          ? user === null
            ? "new_teacher"
            : "old_teacher"
      ...
      if (user === null) {
        ...
        const baseData = { firstName, lastName, email, pictureUrl };
