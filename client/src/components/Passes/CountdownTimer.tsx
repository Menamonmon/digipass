...
const CountdownTimer: React.FC<CountdownTimerProps> = ({duration}) => {
  ...
  <div className="scale-125">
    <CountdownCircleTimer
      ...
      colorsTime={[durationinseconds * 0.75, durationinseconds * 0.5, durationinseconds * 0.25, 0]}
    ...
    initialRemainingTime={remainingTime % minuteSeconds}
        ...
        >
          {({ elapsedTime, color }) => (
            <span style={{ color }}>
              {renderTime(getTimeSeconds(elapsedTime),getTimeMinutes(durationinseconds-elapsedTime))}
            </span>
          )}
